...
public class BioMissleMovement : ProjectileMovement
{
    float velocity;
    Quaternion rotation;
    float time;
    ...
    public override void setTarget(Vector3 target, float accuracy)
    {
        this.target = new Vector3(target.x, target.y + 10, target.z);
        set = true;
        setDirection();
    }
}
